...
void Font::AddStyle(string key, wstring font, float size, DWRITE_TEXT_ALIGNMENT textAlign, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
	if (formats.count(key) > 0) return;

	IDWriteTextFormat* format;

	//writeFactory->CreateTextFormat(font.c_str(), nullptr, weight, style, stretch, size, L"en-US", &format);
...
void Font::RenderText(string text, Float2 pos, Float2 size)
{
	...
	context->DrawTextW(temp.c_str(), temp.size(), curFormat, &rectF, curBrush, D2D1_DRAW_TEXT_OPTIONS_NONE, measuringMode);*/
}
