...
public:
    ...
    void PlayClip(UINT instanceID, int clip, float playRate = 1.0f, float takeTime = 0.1f);

    Matrix GetTransformByNode(UINT instanceID, int nodeIndex);

    Motion* GetMotion(UINT instanceID) { return &frameInstancingBuffer->Get().motions[instanceID]; }
