...
void ModelExporter::SetClipNode(const KeyData& keyData, const UINT& frameCount, ClipNode& clipNode)
{
    ...
    FOR(frameCount)
    {
        ...
        clipNode.transforms[i].rot = CalcInterpolationQuaternion(keyData.rotations, rotCount, i);
...
Float4 ModelExporter::CalcInterpolationQuaternion(const vector<KeyQuat>& keyData, UINT& count, int curFrame)
{
    if (keyData.size() <= count || keyData.size() == 1)
    {
        return keyData.back().value;
