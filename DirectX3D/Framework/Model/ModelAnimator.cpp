...
void ModelAnimator::ReadClip(string clipName, bool isLoop, UINT clipNum, string lockBone, bool rootMotion)
{
...
void ModelAnimator::PlayClip(int clip, float playRate, float takeTime)
{
    isPlay = true;

    frameBuffer->Get().next.clip = clip;
    frameBuffer->Get().next.playRate = playRate;
    frameBuffer->Get().takeTime = takeTime;

    clips[clip]->Init(playRate);
}
...
void ModelAnimator::UpdateFrame()
{
...

    {
        ...
        clip->playTime += frameData.cur.playRate * DELTA;
        frameData.cur.time += clip->tickPerSecond * frameData.cur.playRate * DELTA;

        if (frameData.cur.time >= 1.0f)
        {
            UINT prevFrame = frameData.cur.curFrame;
            frameData.cur.curFrame = (frameData.cur.curFrame + 1);
            ...
            {
                frameData.cur.curFrame = min(frameData.cur.curFrame, clip->frameCount - 2);
                frameData.cur.time -= 1.0f;
            }
        }
...

    {
        ...
        frameData.next.time += clip->tickPerSecond * frameData.next.playRate * DELTA;

        if (frameData.next.time >= 1.0f)
        {
            UINT prevFrame = frameData.next.curFrame;
