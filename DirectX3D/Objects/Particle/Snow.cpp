...
void Snow::GUIRender()
{
    ...
    ImGui::SliderFloat("Turbulance", &buffer->Get().turbulence, 0.0f, 10.0f);
...
void Snow::Create()
{
    ...
    FOR(particleCount)
    {
        ...
        vertices[i].random.x = Random(-1.0f, 1.0f);
        vertices[i].random.y = Random(-1.0f, 1.0f);
    }
